...
figure(100)
load PSS_linearized
sys_linearized=sys_true;
bode(sys_MOSTAR,'g',sys_linearized,'k')
%%
load PSS_ON
load exc_r_I2
r = r/1000;
r = r(1:3301);
plot(t,r,t,w,'LineWidth',1);xlabel('Time (s)','FontSize',13);ylabel('Generator speed deviation (p.u.)','FontSize',13);legend('Speed measurement','Added noise');xlim([0 16.5])
...
figure(4); plot(t,w);xlabel('Time (s)','FontSize',13);ylabel('Generator speed deviation (p.u.)','FontSize',13)
...
Fs = 1/Ts;
L = length(y_est);
FFT_est = fft(y_est); % FFT from estimated model output
...
f = 2*pi*Fs*(0:(L/2))/L;
plot(f,P1) 
title('Single-Sided Amplitude Spectrum of X(t)');xlim([0 50]);
