...
model grid
  Modelica.Blocks.Interfaces.RealOutput Pout
    annotation (Placement(transformation(extent={{-42,16},{-34,24}}),  iconTransformation(
          extent={{-42,16},{-34,24}})));
  ...
  Modelica.Blocks.Interfaces.RealInput EFD1
    ...
    annotation (Placement(transformation(extent={{-192,20},{-152,60}})));
equation
      Pout = Gen1.P;
      Qout = Gen1.Q;
  ...
  connect(infiniteBus.p,bus1. p) annotation (Line(points={{-14,46},{-38,46}},
                             color={0,0,255}));
  connect(Gen1.EFD, EFD1)
    annotation (Line(points={{-138.4,40},{-172,40}}, color={0,0,127}));
